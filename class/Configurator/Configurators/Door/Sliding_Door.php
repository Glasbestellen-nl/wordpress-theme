...
class Sliding_Door extends Configurator {
   ...
   protected function calculate_price_table( array $configuration = [] ) {
      ...
      $price_table = ['rail_width' => 0]; // Key must exist in default price table to include in price
      ...
      if ( ! empty( $configuration['width'] ) && ! empty( $configuration['height'] ) ) {
         ...
         if ( $width > 1250 ) {
            ...
            $price_table['rail_width'] = $this->get_metadata( 'rail_1250' );
         }
      }
