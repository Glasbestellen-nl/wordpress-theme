...
jQuery(document).ready(function ($) {
  ...
  $(window).on("load", function () {
    ...
    if ("crm" == page) {
      const leadID = urlParams.get("lead_id"); // Lead single screen
    ...
    $("textarea.js-tab-support").keydown(function (e) {
      ...
      if (e.keyCode === 13 && enabled) {
        ...
        if (this.selectionStart == this.selectionEnd) {
          ...
          if (sel > lineStart) {
            ...
            document.execCommand("insertText", false, "\n" + text.substr(lineStart, sel - lineStart)); // Scroll caret visible
      ...
      if (e.keyCode === 9 && enabled) {
        ...
        if (this.selectionStart == this.selectionEnd) {
          ...
          while (selEnd > 0 && text[selEnd - 1] != "\n" && selEnd < text.length) selEnd++; // Get selected text
          ...
          for (var i = 0; i < lines.length; i++) {
            ...
            if (i == lines.length - 1 && lines[i].length == 0) continue; // Tab or Shift+Tab?

            if (e.shiftKey) {
              if (lines[i].startsWith("\t")) lines[i] = lines[i].substr(1);else if (lines[i].startsWith("    ")) lines[i] = lines[i].substr(4);
