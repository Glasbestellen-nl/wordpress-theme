const Configurator = (function () {
  ...
  function ConfiguratorObject(element) {
        ...
        .on(
          ...
          function () {
            ...
            if (form) {
              ...
              $(":input:visible, .js-input-hidden", form).each(function (
                index,
                element
              ) {
    ...
    this.validateForm = function (form, callback) {
      ...
      if (valid) {
        ...
        if (!firstInput.isInViewport()) firstInput.scrollTo(-200);
      }
    };
    ...
    this.validateInput = function (element) {
      ...
      let value =
        ($(element).is("select") &&
          $(element).find(":selected").data("option-value")) ||
        element.value;
        ...
        if (rules) {
          ...
          if (rules.max !== undefined) {
            ...
            if (rules.max.dependence !== undefined) {
              ...
              dependentStepIds =
                (!Array.isArray(dependentStepIds) && [dependentStepIds]) ||
                dependentStepIds;

              let previousMax = 0;
              dependentStepIds.forEach((dependentStepId) => {
                let dependentValue = $(".js-step-" + dependentStepId)
                  .find(".js-form-validate")
                  .val();
          ...
          if (rules.less_than && rules.less_than.step) {
            ...
            let dependentStep = $(".js-step-" + dependentStepId).find(
              ".js-form-validate"
            );
            let dependentValue =
              parseInt(
                dependentStep.is("select") &&
                  dependentStep.find(":selected").data("option-value")
              ) || parseInt(dependentStep.val());
  ...
  return {
    getInstance: (element) => {
      if (!instance) instance = createInstance(element);
      return instance;
    },
  };
})();
